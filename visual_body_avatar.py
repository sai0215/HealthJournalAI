...
# --- Page Configuration ---
st.set_page_config(layout="wide", page_title="Visual Human Body Avatar")
...
st.markdown("""
<div style="text-align: center; font-family: monospace; font-size: 20px; line-height: 1.4; background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); padding: 30px; border-radius: 20px; border: 4px solid #007bff; margin: 25px 0; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
<pre style="margin: 0; font-size: 18px; color: #2c3e50;">
                           🧠 HEAD
                          /   \\
                         /     \\
                    🦴---🫁---🦴
...
st.markdown("""
<div style="text-align: center; font-family: monospace; font-size: 18px; background: linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%); padding: 25px; border-radius: 15px; border: 3px solid #2196f3; margin: 20px 0; box-shadow: 0 3px 6px rgba(0,0,0,0.1);">
<strong style="font-size: 20px; color: #1976d2;">ORGANS LOCATION:</strong><br><br>
<span style="font-size: 24px;">❤️ Heart</span>    <span style="font-size: 24px;">🫁 Lungs</span>    <span style="font-size: 24px;">🫀 Liver</span>    <span style="font-size: 24px;">🫄 Stomach</span><br>
<span style="font-size: 24px;">🫘 Kidneys</span>  <span style="font-size: 24px;">🫀 Bladder</span>  <span style="font-size: 24px;">🧠 Brain</span>    <span style="font-size: 24px;">🦴 Shoulders</span><br>
<span style="font-size: 24px;">💪 Arms</span>     <span style="font-size: 24px;">🦵 Legs</span>
</div>
""", unsafe_allow_html=True)
...
# Create body part buttons in anatomical layout
...
for i, (part_key, part_data) in enumerate(body_parts_preview.items()):
    with cols[i % 7]:
        st.markdown(f"""
        <div style="text-align: center; padding: 15px; border: 3px solid #007bff; border-radius: 12px; margin: 5px; background: linear-gradient(135deg, #e3f2fd 0%, #f8f9fa 100%); box-shadow: 0 3px 6px rgba(0,0,0,0.15);">
            <div style="font-size: 28px; margin-bottom: 8px;">{part_data['emoji']}</div>
            <div style="font-size: 12px; font-weight: bold; color: #2c3e50; margin-bottom: 5px;">{part_data['name']}</div>
            <div style="font-size: 16px; color: #28a745; font-weight: bold; margin-bottom: 3px;">{part_data['score']}</div>
            <div style="font-size: 12px; color: #6c757d;">{part_data['status']}</div>
        </div>
        """, unsafe_allow_html=True)

# --- Selected Body Part Information ---
